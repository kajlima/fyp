...
# same epsilon as the final FGSM RF attack (fgsm_attack.py used 1.5)
# these are the single source of truth; the CLI defaults are built from them below
...
# ============================================================
# FEATURE SET (must match the MLP/RF training)
# ============================================================
FULL_FEATURES = [
    # static / enrolment
    "degree_size",
...
# ============================================================
# SOURCE MLP MODEL (must match the training MLP exactly)
# ============================================================
...
def get_selected_features(feature_set):
    ...
    if feature_set != "full":
        raise ValueError("this script only supports feature_set='full'.")
...
def prepare_features_and_label(df, feature_set):
    ...
    if missing_features:
        raise ValueError(f"feature set {feature_set} needs missing columns:\n{missing_features}")
...
def load_rf_best_params(rf_summary_path):
    ...
    if best_params is None:
        return default_params, "default_params"
    # keep only the params the RF actually uses
...
def train_defended_rf(rf_baseline, X_train_mixed, y_train_mixed, random_state):
    # clone the baseline RF pipeline so the defended model keeps the exact same
    # preprocessing and tuned hyperparameters, then refit it on clean + adversarial data
...
def run_single_adversarial_training_experiment(
    ...
    if mlp_info["selected_features"] is not None and list(mlp_info["selected_features"]) != list(selected_features):
        raise ValueError(
            "selected_features in the MLP checkpoint do not match this script.\n"
            "make sure --csv-path and --feature-set match the MLP training."
        )
    ...
    # baseline clean predictions used to generate the training attack samples
    ...
    # save the before-defence confusion matrices for test and validation
...
def build_arg_parser():
    ...
    parser.add_argument("--rf-results-dir", default=DEFAULT_RF_RESULTS_DIR, help=f"baseline RF results folder. default: {DEFAULT_RF_RESULTS_DIR}")
    parser.add_argument("--mlp-dir", default=DEFAULT_MLP_DIR, help=f"source MLP results folder. default: {DEFAULT_MLP_DIR}")
    parser.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR, help=f"defended RF output folder. default: {DEFAULT_OUTPUT_DIR}")
    ...
    parser.add_argument(
        ...
        help="untargeted = general degradation. target_non_dropout = targeted push toward non-dropout.",
    )
    parser.add_argument(
        "--train-attack-scope",
    ...
    parser.add_argument(
        ...
        choices=["all", "dropout_only", "true_positive_only"],
        help="scope used for the final FGSM evaluation. default: true_positive_only.",
    )
    parser.add_argument(
        "--attack-scope",
        default=None,
        choices=["all", "dropout_only", "true_positive_only"],
        help="backward-compatible alias. if set, it applies to both train and eval scope.",
    )
    parser.add_argument(
        "--train-epsilons",

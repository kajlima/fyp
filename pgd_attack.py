#!/usr/bin/env python3
"""
PGD attack directly on the trained/source MLP model.

Designed for the normal MLP output folder, for example:
    mlp_results/
        mlp_model.pt
...
Key idea:
- This attacks the MLP itself, not Random Forest.
- PGD uses gradients from the MLP main model.
- By default, the attack is targeted toward class 0 = non-dropout.
- By default, only clean true-positive dropout rows are perturbed.
- Only the allowed raw features are changed; all other features are frozen.
"""
...
DEFAULT_OUTPUT_DIR = "pgd_mlp_full_results"

TARGET_COL = "target"
NEGATIVE_LABEL = 0  # non-dropout
POSITIVE_LABEL = 1  # dropout

# Use epsilon 1.0 by default for PGD MLP attack.
EPSILON_LIST = [1.0]

# For eps=1.0, use a softer step.
# 0.01 x 90 gives a controlled attack instead of collapsing F1 too aggressively.
PGD_STEP = 0.01
PGD_ITERATIONS = 90
...
# Only these raw features are allowed to move.
# Flags, categorical features, and all other raw features are frozen.
ATTACKABLE_FEATURES = [
    "credits_enrolled_semester_a",
    "credits_enrolled_semester_b",
    "total_credits_enrolled_academic_year",
...
def split_70_20_10_stratified(X, y, student_ids=None, random_state=42):
    """
    Stratified random split with no grouping. This is identical to the split
    used by the Random Forest and MLP training scripts:
    ...
    across all splits. Each row is treated as an independent observation. Using
    the same split logic, the same input file, and the same random_state as the
    MLP training script guarantees that the test/validation rows here are exactly
    the rows the MLP was evaluated on, and are disjoint from its training rows.
    """
...
def resolve_mlp_model_path(mlp_results_dir, feature_set):
    candidates = [
        os.path.join(mlp_results_dir, "mlp_model.pt"),
        os.path.join(mlp_results_dir, feature_set, "mlp_model.pt"),
    ]
...
def resolve_mlp_preprocessor_path(mlp_results_dir, feature_set):
    candidates = [
        os.path.join(mlp_results_dir, "mlp_preprocessor.pkl"),
        os.path.join(mlp_results_dir, feature_set, "mlp_preprocessor.pkl"),
    ]
    for path in candidates:
        if os.path.exists(path):
            return path
...
def resolve_mlp_summary_path(mlp_results_dir, feature_set):
    candidates = [
        os.path.join(mlp_results_dir, "mlp_summary.json"),
        os.path.join(mlp_results_dir, feature_set, "mlp_summary.json"),
    ]
    for path in candidates:
        if os.path.exists(path):
            return path
...
def load_summary_threshold(summary_path):
    ...
    with open(summary_path, "r", encoding="utf-8") as f:
        summary = json.load(f)
    threshold = find_threshold_in_dict(summary)
    if threshold is None:
        return None, summary_path
    return float(threshold), summary_path
...
def load_mlp_model(model_path, input_dim_from_data, device, summary_threshold=None):
    ...
    if input_dim != input_dim_from_data:
        raise ValueError(f"Input dim model ({input_dim}) beda dengan data ({input_dim_from_data}).")
...
def make_preprocessed_attack_mask(preprocessor, selected_features, feature_set, input_dim, output_dir):
    """
    Build ART mask in transformed/preprocessed space.

    ColumnTransformer output order is expected to be:
    numeric block, flag block, categorical one-hot block.
    Only numeric transformed columns corresponding to allowed raw features are attackable.
    """
    ...
    if len(numeric_cols) > input_dim:
        raise ValueError("Jumlah numeric columns lebih besar dari input_dim preprocessed.")
...
def preprocessed_adv_to_raw_and_scaled(
    ...
    Only the numeric block is inverse-transformed with StandardScaler.
    Frozen raw features are restored from clean raw data.
    Non-attacked rows are restored completely.
    """
    attackable_raw = set(ATTACKABLE_FEATURES)
    numeric_cols = get_numeric_cols_from_preprocessor(preprocessor)
    ...
    # Restore non-attacked rows completely, so attack_scope is strict.
    # Restore column-by-column to avoid pandas dtype FutureWarning.
...
def predict_mlp_proba(model, X_preprocessed, device, batch_size=4096):
    ...
    with torch.no_grad():
        for start in range(0, len(X_preprocessed), batch_size):
            batch = torch.tensor(
                X_preprocessed[start:start + batch_size],
                dtype=torch.float32,
                device=device,
            )
            logits = model(batch)
...
def evaluate_mlp_with_threshold(model, X_preprocessed, y_true, threshold, condition, epsilon, split, device):
...
def save_attack_predictions(
    ...
    pred_df = pd.DataFrame({
        ...
        "mlp_clean_pred_label": clean_pred,
        "mlp_clean_pred_target": np.where(clean_pred == 1, "dropout", "non-dropout"),
        "mlp_adv_pred_label": adv_pred,
        "mlp_adv_pred_target": np.where(adv_pred == 1, "dropout", "non-dropout"),
        "mlp_clean_prob_dropout": clean_proba,
        "mlp_adv_prob_dropout": adv_proba,
...
def save_clean_vs_adv_sample(
    ...
    for i in range(n):
        row = {
            ...
            "mlp_clean_probability_dropout": float(clean_proba[i]),
            "mlp_adversarial_probability_dropout": float(adv_proba[i]),
            "mlp_clean_prediction": int(clean_pred[i]),
            "mlp_adversarial_prediction": int(adv_pred[i]),
...
def make_attack_labels(attack_mode, y_true):
    if attack_mode == "untargeted":
        # Untargeted PGD: maximize loss against TRUE labels.
        return one_hot(y_true), False

    if attack_mode == "target_non_dropout":
        # Targeted PGD: push selected rows toward class 0 = non-dropout.
...
def run_single_pgd_mlp_experiment(
    df,
    csv_path,
    feature_set,
    output_dir,
    ...
    print("=" * 80)
    print("PGD ATTACK DIRECTLY ON MLP MAIN")
    print("=" * 80)
    ...
    model_path = resolve_mlp_model_path(mlp_results_dir, feature_set)
    preprocessor_path = resolve_mlp_preprocessor_path(mlp_results_dir, feature_set)
    summary_path = resolve_mlp_summary_path(mlp_results_dir, feature_set)
    summary_threshold, summary_threshold_source = load_summary_threshold(summary_path)
    ...
    model, mlp_info = load_mlp_model(
        model_path=model_path,
        input_dim_from_data=input_dim,
        device=device,
    ...
    if mlp_info["selected_features"] is not None and list(mlp_info["selected_features"]) != list(selected_features):
        raise ValueError(
            "selected_features di checkpoint MLP tidak sama dengan selected_features script PGD.\n"
            "Pastikan --csv-path dan --feature-set sama dengan training MLP."
        )

    art_mlp = wrap_mlp_for_art(model, input_dim=input_dim)
    ...
    if summary_path:
        print("Loaded MLP summary:")
        print(summary_path)
    print(f"MLP threshold: {mlp_info['threshold']} | source: {mlp_info['threshold_source']}")
    print(f"Hidden dims  : {mlp_info['hidden_dims']}")
    print(f"Dropout      : {mlp_info['dropout']}")
    print()
    ...
    clean_test_result, clean_test_cm, clean_test_pred, clean_test_proba = evaluate_mlp_with_threshold(
        model=model,
        X_preprocessed=X_test_preprocessed,
        y_true=y_test.values,
        threshold=mlp_info["threshold"],
        condition="Clean",
        epsilon=0.0,
        split="test",
        device=device,
    ...
    clean_val_result, clean_val_cm, clean_val_pred, clean_val_proba = evaluate_mlp_with_threshold(
        model=model,
        X_preprocessed=X_val_preprocessed,
        y_true=y_val.values,
        threshold=mlp_info["threshold"],
        condition="Clean",
        epsilon=0.0,
    ...
    save_confusion_matrix(clean_test_cm, os.path.join(output_dir, "mlp_clean_test_confusion_matrix.csv"))
    save_confusion_matrix(clean_val_cm, os.path.join(output_dir, "mlp_clean_validation_confusion_matrix.csv"))
    ...
    for epsilon in epsilon_list:
        ...
        X_test_adv_pre_raw, test_attack_idx = generate_pgd_for_scope(
            pgd=pgd,
        ...
        X_val_adv_pre_raw, val_attack_idx = generate_pgd_for_scope(
            pgd=pgd,
        ...
        test_result, test_cm, test_adv_pred, test_adv_proba = evaluate_mlp_with_threshold(
            model=model,
            X_preprocessed=X_test_adv_preprocessed,
            y_true=y_test.values,
            threshold=mlp_info["threshold"],
            condition=f"PGD {attack_mode}",
            epsilon=epsilon,
            split="test",
            device=device,
        )

        val_result, val_cm, val_adv_pred, val_adv_proba = evaluate_mlp_with_threshold(
            model=model,
            X_preprocessed=X_val_adv_preprocessed,
            y_true=y_val.values,
            threshold=mlp_info["threshold"],
            condition=f"PGD {attack_mode}",
            epsilon=epsilon,
        ...
        test_result["pgd_step"] = effective_step
        test_result["pgd_iterations"] = int(pgd_iterations)
        ...
        val_result["pgd_step"] = effective_step
        val_result["pgd_iterations"] = int(pgd_iterations)
        ...
        test_stats = perturbation_stats(
            X_clean_preprocessed=X_test_preprocessed,
            X_adv_preprocessed=X_test_adv_preprocessed,
            perturbable_mask=perturbable_mask,
            attacked_row_mask=test_attack_idx,
        )
        test_stats.update({
        ...
        val_stats = perturbation_stats(
            X_clean_preprocessed=X_val_preprocessed,
            X_adv_preprocessed=X_val_adv_preprocessed,
            perturbable_mask=perturbable_mask,
            attacked_row_mask=val_attack_idx,
        )
        val_stats.update({
        ...
        np.save(os.path.join(output_dir, f"X_test_pgd_preprocessed_eps_{eps_tag}.npy"), X_test_adv_preprocessed)
        np.save(os.path.join(output_dir, f"X_validation_pgd_preprocessed_eps_{eps_tag}.npy"), X_val_adv_preprocessed)

        X_test_adv_raw.to_csv(os.path.join(output_dir, f"X_test_pgd_raw_eps_{eps_tag}.csv"), index=False)
        X_val_adv_raw.to_csv(os.path.join(output_dir, f"X_validation_pgd_raw_eps_{eps_tag}.csv"), index=False)

        save_confusion_matrix(test_cm, os.path.join(output_dir, f"mlp_pgd_test_confusion_matrix_eps_{eps_tag}.csv"))
        save_confusion_matrix(val_cm, os.path.join(output_dir, f"mlp_pgd_validation_confusion_matrix_eps_{eps_tag}.csv"))
        ...
        save_attack_predictions(
            ...
            epsilon=epsilon,
            split="test",
            path=os.path.join(output_dir, f"mlp_pgd_test_predictions_eps_{eps_tag}.csv"),
        )
        ...
        save_attack_predictions(
            ...
            epsilon=epsilon,
            split="validation",
            path=os.path.join(output_dir, f"mlp_pgd_validation_predictions_eps_{eps_tag}.csv"),
        )
        ...
        save_clean_vs_adv_sample(
            ...
            epsilon=epsilon,
            split="test",
            path=os.path.join(output_dir, f"clean_vs_pgd_test_sample_eps_{eps_tag}.csv"),
            max_samples=50,
        )
        ...
        save_clean_vs_adv_sample(
            ...
            epsilon=epsilon,
            split="validation",
            path=os.path.join(output_dir, f"clean_vs_pgd_validation_sample_eps_{eps_tag}.csv"),
            max_samples=50,
        )
    ...
    results_path = os.path.join(output_dir, "mlp_pgd_attack_results.csv")
    stats_path = os.path.join(output_dir, "mlp_pgd_perturbation_stats.csv")
    ...
    summary = {
        ...
        "feature_set": feature_set,
        "output_dir": output_dir,
        "attack": "PGD directly against MLP Main",
        "attack_mode": attack_mode,
        "attack_scope": attack_scope,
        "epsilons": epsilon_list,
        "pgd_step": pgd_step,
        "pgd_iterations": pgd_iterations,
        "target_model": "MLP Main",
        "gradient_model": "MLP Main itself, no surrogate",
        "mlp_model_path": model_path,
        "mlp_preprocessor_path": preprocessor_path,
        "mlp_summary_path": summary_path,
        "mlp_threshold": mlp_info["threshold"],
        "mlp_threshold_source": mlp_info["threshold_source"],
        "hidden_dims": list(mlp_info["hidden_dims"]),
    ...
    summary_path_out = os.path.join(output_dir, "mlp_pgd_attack_summary.json")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="PGD attack directly on the trained/source MLP model."
    )
    ...
    parser.add_argument(
        "--mlp-results-dir",
        default=DEFAULT_MLP_RESULTS_DIR,
        help=f"Folder hasil MLP. Default: {DEFAULT_MLP_RESULTS_DIR}",
    )

    # Alias for convenience.
    parser.add_argument(
        "--mlp-dir",
        dest="mlp_results_dir",
        help="Alias untuk --mlp-results-dir.",
    )

    parser.add_argument(
        "--output-dir",
        default=DEFAULT_OUTPUT_DIR,
        help=f"Folder output PGD. Default: {DEFAULT_OUTPUT_DIR}",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "untargeted = general degradation. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "true_positive_only = attack hanya dropout yang clean MLP-nya benar terdeteksi dropout."
        ),
    )
    ...
    parser.add_argument(
        "--epsilons",
        default="1.0",
        help="Comma-separated epsilon list. Default: 1.0",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=PGD_STEP,
        help=f"PGD eps_step. Default: {PGD_STEP}",
    )

    parser.add_argument(
        "--pgd-iterations",
        type=int,
        default=PGD_ITERATIONS,
        help=f"PGD max_iter. Default: {PGD_ITERATIONS}",
    )

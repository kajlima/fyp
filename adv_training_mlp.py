#!/usr/bin/env python3
"""
MLP adversarial training (defence) with PGD, FULL feature set only.

Config is aligned with the PGD attack we actually ran:
- Dataset default: zenodo.csv
- Baseline MLP folder default: mlp_results
- Feature set: full
- Attack: targeted PGD toward class 0 = non-dropout
...
- Evaluation attack source: defended (adaptive white-box PGD on the defended MLP)
- PGD epsilon (train and eval): 1.0
- PGD step size: 0.01
- PGD iterations: 90
- Split: stratified 70/20/10, same as the baseline MLP.
...
# ============================================================
# FULL FEATURE SET - MUST MATCH YOUR MLP TRAINING SCRIPT
# ============================================================
...
def make_criterion(class_weight, y_train, device):
    ...
    if class_weight == "balanced":
        ...
        if np.any(counts == 0):
            raise ValueError(f"Class count tidak valid: {counts}")
...
def prepare_features_and_label(df):
    ...
    if missing_features:
        raise ValueError(f"Feature set full butuh kolom yang tidak ada:\n{missing_features}")
...
def load_mlp_checkpoint(model_path, input_dim_from_data, device, summary_threshold=None):
    ...
    if selected_features is not None and list(selected_features) != FULL_FEATURES:
        raise ValueError("selected_features di checkpoint tidak sama dengan FULL_FEATURES script ini.")
...
def make_preprocessed_attack_mask(preprocessor, input_dim, output_dir):
    ...
    if missing_attackable:
        raise ValueError(f"Attackable features tidak ada di FULL_FEATURES:\n{missing_attackable}")
    ...
    if int(mask.sum()) == 0:
        raise ValueError("Tidak ada preprocessed feature yang bisa diserang.")
...
# ============================================================
# ATTACK GENERATION WITH TRUE-POSITIVE-ONLY SCOPE
# ============================================================
...
def run_experiment(
    ...
    for train_epsilon in train_epsilon_list:
        ...
        for split_name, X_pre_split, y_split, id_split in [
            ...
            # Save clean predictions with adv columns equal to clean for consistent schema.
            save_predictions(
        ...
        if eval_attack_source == "defended":
            ...
            raise ValueError("eval_attack_source harus 'defended' atau 'baseline'.")
...
def build_arg_parser():
    ...
    parser.add_argument("--feature-set", default="full", choices=["full"], help="Only full is supported in this final script.")
    parser.add_argument("--mlp-results-dir", default=DEFAULT_MLP_RESULTS_DIR, help=f"Folder hasil MLP baseline. Default: {DEFAULT_MLP_RESULTS_DIR}")
    parser.add_argument("--mlp-dir", dest="mlp_results_dir", help="Alias untuk --mlp-results-dir.")
    parser.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR, help=f"Folder output defended MLP. Default: {DEFAULT_OUTPUT_DIR}")
    ...
    parser.add_argument("--eval-attack-scope", default="true_positive_only", choices=["all", "dropout_only", "true_positive_only"], help="Scope used for final PGD evaluation. Default: true_positive_only.")
    parser.add_argument("--attack-scope", default=None, choices=["all", "dropout_only", "true_positive_only"], help="Backward-compatible alias. If provided, it sets both train and eval attack scope.")
    parser.add_argument("--eval-attack-source", default="defended", choices=["defended", "baseline"], help="defended = adaptive white-box PGD against defended MLP.")
    parser.add_argument("--train-epsilons", default="1.0", help="Comma-separated train epsilon list. Default: 0.2")
    parser.add_argument("--eval-epsilons", default="1.0", help="Comma-separated eval epsilon list. Default: 0.05")
...
def main():
    ...
    if args.feature_set != "full":
        raise ValueError("Script final ini hanya untuk --feature-set full.")

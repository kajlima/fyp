...
# softer step for eps=1.0: 0.01 x 100 keeps the attack controlled instead of
# collapsing F1 too aggressively
PGD_STEP = 0.01
PGD_ITERATIONS = 100
...
def split_70_20_10_stratified(X, y, student_ids=None, random_state=42):
    ...
    # rows here are exactly the rows the MLP was evaluated on
...
def make_preprocessed_attack_mask(preprocessor, selected_features, feature_set, input_dim, output_dir):
    ...
    if len(numeric_cols) > input_dim:
        raise ValueError("Number of numeric columns exceeds preprocessed input_dim.")
...
def make_attack_labels(attack_mode, y_true):
    if attack_mode == "untargeted":
        # untargeted PGD: maximize loss against the TRUE labels
...
def make_attack_scope_mask(attack_scope, y_true, clean_pred):
    ...
    # true_positive_only = true dropout rows MLP already detected (cleanest evasion)
    y_true = np.asarray(y_true).astype(int)
...
def generate_pgd_for_scope(
    ...
    # run PGD only on the selected rows; untouched rows stay clean
...
def run_single_pgd_mlp_experiment(
    ...
    # load MLP (target + gradient model), its preprocessor, and tuned threshold
    ...
    # clean MLP baseline on test and validation
    ...
    for epsilon in epsilon_list:
        ...
        # keep eps_step no larger than eps
        ...
        # generate PGD in preprocessed space for the chosen scope
        ...
        # evaluate the adversarial examples on MLP
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="PGD attack directly on the trained MLP model."
    )
    parser.add_argument(
        "--csv-path",
    ...
    parser.add_argument(
        ...
        help=f"MLP results folder. Default: {DEFAULT_MLP_RESULTS_DIR}",
    )
    # convenience alias
    parser.add_argument(
        "--mlp-dir",
        dest="mlp_results_dir",
        help="Alias for --mlp-results-dir.",
    )
    parser.add_argument(
        "--output-dir",
        default=DEFAULT_OUTPUT_DIR,
        help=f"PGD output folder. Default: {DEFAULT_OUTPUT_DIR}",
    )
    parser.add_argument(
        "--random-state",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "true_positive_only = attack only dropout rows MLP already detected as dropout."
        ),
    )
    parser.add_argument(
        "--epsilons",

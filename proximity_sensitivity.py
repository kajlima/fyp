#!/usr/bin/env python3
"""
Proximity and sensitivity analysis for the Zenodo FULL-feature experiments.

Aligned with the finalized setup:
- Dataset: zenodo.csv
- Feature set: full
...
2. Sensitivity: sum over numeric features of |adv - clean| / sigma_j, averaged over
   samples (Eq. 5 and 6). Numeric features are StandardScaler-scaled, so the absolute
   preprocessed difference already equals |adv - clean| / sigma_j.
3. Frozen-feature integrity.
4. Dropout hiding rate: true-positive dropout cases flipped to non-dropout.
5. Metrics for both all rows and attacked rows only.
"""
...
def candidate_eps_strings(value: float) -> List[str]:
    # supports both new p-format and old decimal format
    raw = str(value)
...
def load_and_split_dataset(csv_path: str, random_state: int):
    ...
    # Stratified 70/20/10 split, identical to the RF/MLP training and defence scripts
    # (same test_size, stratify, and random_state). This guarantees the test/validation
    # rows and their order here match the adversarial arrays saved by the defence scripts,
    # so clean and adversarial samples line up row by row.
...
def get_transformer_slice(preprocessor, transformer_name: str, input_dim: int, fallback_start: int = 0):
    ...
    # Fallback: assume numeric block first.
...
def make_preprocessed_masks(preprocessor, input_dim: int, output_dir: str):
    ...
    # Numeric block: attackable raw numeric features only.
    ...
    # Treat other blocks as categorical/OHE for sensitivity purposes.
    # They should be frozen in the final attacks.
    ...
    # If no categorical mask was found, mark non-numeric dimensions as categorical/frozen.
...
def compute_sensitivity_metrics(
    ...
    # Sensitivity (Eq. 5 and 6): SEN = sum over numeric features of |x_adv - x_clean| / sigma_j.
    # The numeric block is StandardScaler output, so |z_adv - z_clean| already equals
    # |x_adv - x_clean| / sigma_j (sigma_j = training std). We therefore sum the absolute
    # numeric differences per sample (a SUM over features, no extra division by std), then
    # average over samples for the dataset-level value.
    if num_mask.sum() > 0:
        per_sample_sen = abs_diff[:, num_mask].sum(axis=1)
    else:
        per_sample_sen = np.zeros(len(Xc), dtype=np.float32)

    # Diagnostic only (not part of the sensitivity formula): fraction of categorical/OHE
    # dimensions that changed. Should be ~0 because those features are frozen in the attacks.
    if cat_mask.sum() > 0:
...
def feature_sensitivity_summary(
    ...
    # Per-feature sensitivity term consistent with Eq. 5: |x_adv - x_clean| / sigma_j.
    # Numeric block is StandardScaler output, so the absolute numeric difference already
    # equals |x_adv - x_clean| / sigma_j. Categorical/OHE features are not part of the
    # sensitivity formula and are left at zero here.
...
def process_final_case(
    ...
    if pre_path is not None:
        ...
        # Ensure column order matches FULL_FEATURES.
...
def main():
    ...
    # Build masks from test split dimension.
    ...
    if not all_rows:
        raise FileNotFoundError(
            "No proximity/sensitivity rows were created. Check that the final RF/MLP defence output folders exist "

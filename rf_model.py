...
def get_selected_features_for_feature_set(feature_set: str) -> list:
    """Return the exact full feature list for the RF experiment."""
    if feature_set != "full":
...
def prepare_features_and_label(df, feature_set):
    ...
    # ============================================================
    # This script only runs the full-feature model.
    feature_set = "full"
...
def infer_rf_c_feature_groups(X):
    """
    Build RF-C preprocessing groups aligned with MLP-C.

    - seniority is one-hot encoded even if it is numeric.
    - *_flag columns are kept as binary indicators and not mixed with ordinary numeric features.
    - remaining numeric/bool columns are median-imputed.
    ...
    if unassigned_cols:
        raise ValueError(
            "Some features were not assigned to an RF-C preprocessing group: "
            f"{unassigned_cols}"
        )
...
def build_preprocessor(X):
    numeric_cols, flag_cols, categorical_cols = infer_rf_c_feature_groups(X)
...
def run_single_rf_experiment(df, csv_path, output_dir, feature_set, random_state):
    """
    Run one RF experiment using FULL_FEATURES only.

    Split usage:
    - train set: model fitting
    - test set: hyperparameter and threshold selection
    ...
    print("RF-C preprocessing groups:")
    ...
    best_threshold_df.to_csv(
        os.path.join(output_dir, "rf_threshold_tuning_test.csv"),
        index=False,
    )
    ...
    # Output order: train, test selection, final validation.
    ...
    save_confusion_matrix(
        test_cm,
        os.path.join(output_dir, "rf_test_confusion_matrix.csv"),
    )
    save_confusion_matrix(
        val_cm,
        os.path.join(output_dir, "rf_validation_confusion_matrix.csv"),
    )
    ...
    summary = {
        ...
        "split": {
            ...
            "important_note": "The merged dataset is split 70/20/10 and stratified by label. Each row is treated as an independent observation; no grouping by student_id is applied.",
        },
        "model": "RandomForestClassifier",
        "preprocessing": {
            "policy": "RF-C: numeric values are median-imputed, *_flag columns are kept as binary indicators, and seniority/categorical columns are one-hot encoded.",
            "numeric": "SimpleImputer(strategy='median') fitted on train only; no scaling because Random Forest does not need StandardScaler",
            "binary_flags": "SimpleImputer(strategy='most_frequent'); kept as 0/1 indicators",
        ...
        "final_evaluation": {
            ...
            "note": "The validation set is reserved as the final held-out evaluation set after model and threshold selection on the test set.",
            "metrics_used": [
                "accuracy_score",
                "precision_score",
                "recall_score",

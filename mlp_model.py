...
# - remaining numeric columns are median-imputed and standardized.
...
FEATURE_SET_DESCRIPTIONS = {
    "full": "FULL_FEATURES only: end-of-year/full-feature time point.",
}
...
def validate_feature_set_definitions() -> dict:
    """Validate that the full feature list has no duplicated columns."""
...
def get_selected_features_for_feature_set(feature_set: str) -> list:
    """Return the exact full feature list for the MLP experiment."""
    if feature_set != "full":
...
def save_feature_set_metadata(output_dir, feature_set, selected_features, df_columns):
    """Save a JSON record proving exactly which full-feature logic was used."""
    ...
    meta = {
        **feature_meta,
        ...
        "leakage_note": (
            "Only FULL_FEATURES are used. student_id, academic_year, and target are not predictors."
        ),
    }
...
def prepare_features_and_label(df, feature_set):
    ...
    if missing_features:
        raise ValueError(
            f"Feature set '{feature_set}' requires columns that are missing from the dataset: "
            f"{missing_features}"
        )
...
def split_70_20_10_stratified(X, y, student_ids=None, random_state=42):
    ...
    one unique student_id per row. This uses the same split logic and random
    state as the Random Forest script so both models can use identical
...
def infer_mlp_c_feature_groups(X):
    """
    Build MLP-C feature groups.

    This keeps the information already present in the dataset, but changes how
    the columns are encoded for MLP:
    - seniority is one-hot encoded so MLP does not assume a linear effect.
    - *_flag columns are passed as binary indicators, not standardized.
    - other numeric columns are standardized.
    - object/category columns are one-hot encoded.
    """
    all_cols = X.columns.tolist()

    explicit_onehot_cols = [
        col for col in ONEHOT_CATEGORICAL_FEATURES
        if col in X.columns
    ]

    object_categorical_cols = X.select_dtypes(
        exclude=["number", "bool"]
    ).columns.tolist()
    ...
    if unassigned_cols:
        raise ValueError(
            "Some features were not assigned to an MLP-C preprocessing group: "
            f"{unassigned_cols}"
        )
...
def build_preprocessor(X):
    numeric_cols, flag_cols, categorical_cols = infer_mlp_c_feature_groups(X)
    ...
    if not transformers:
        raise ValueError("No columns are available for preprocessing.")
...
def compute_metrics_with_threshold(model, X_np, y, split_name, threshold, device):
    ...
    result = {
        ...
        "precision_dropout": precision_score(
            y_np,
            y_pred,
            pos_label=POSITIVE_LABEL,
            zero_division=0,
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        default="full",
        choices=["full"],
        help="Feature set to use. This script only runs FULL_FEATURES.",
    )
...
def run_single_mlp_experiment(df, csv_path, output_dir, feature_set, random_state):
    """
    Run one MLP experiment using FULL_FEATURES only.

    Split usage:
    - train set: model fitting
    - test set: hyperparameter selection, early stopping, and threshold selection
    - validation set: final held-out evaluation
    """
    ...
    print("=" * 80)
    print("MLP-C - STRATIFIED SPLIT + FLAG/SENIORITY PREPROCESSING + REGULARIZED TUNING")
    print("=" * 80)
    ...
    print("MLP-C preprocessing groups:")
    ...
    print("Early stopping uses test-selection loss.")
    print("The threshold is selected from the test set.")
    print("The validation set is reserved for final held-out evaluation.")
    print("Model selection uses an overfitting penalty:")
    ...
    for combo_idx, combo in enumerate(combos, start=1):
        ...
        threshold, threshold_df, threshold_best_row = tune_threshold_from_proba(
            y_test_np,
            test_proba_tuning,
        )
        ...
        row = {
            ...
            "test_precision_dropout": precision_score(
                y_test_np,
                test_pred_tuning,
                pos_label=POSITIVE_LABEL,
                zero_division=0,
            ),
            "test_recall_dropout": recall_score(
                y_test_np,
                test_pred_tuning,
                pos_label=POSITIVE_LABEL,
                zero_division=0,
    ...
    pd.DataFrame(best_history).to_csv(
        os.path.join(output_dir, "mlp_training_history.csv"),
        index=False,
    )
    ...
    # Confusion matrices for the test (selection) and validation (final) splits.
    # Rows are the actual class, columns are the predicted class, in the order
    # [non-dropout, dropout], matching the label coding used everywhere else.
    ...
    save_confusion_matrix(
        test_cm,
        os.path.join(output_dir, "mlp_test_confusion_matrix.csv"),
    )
    save_confusion_matrix(
        val_cm,
        os.path.join(output_dir, "mlp_validation_confusion_matrix.csv"),
    )
    ...
    summary = {
        ...
        "split": {
            ...
            "important_note": "The merged dataset is split 70/20/10 and stratified by label. Each row is treated as an independent observation; no grouping by student_id is applied. This uses the same split logic and random_state as the Random Forest script.",
            "split_usage": {
                "train": "model fitting",
        ...
        "preprocessing": {
            "policy": "MLP-C: numeric values are scaled, *_flag columns are kept as binary indicators, and seniority/categorical columns are one-hot encoded.",
            "numeric": "SimpleImputer(strategy='median') + StandardScaler fitted on train only",
            "binary_flags": "SimpleImputer(strategy='most_frequent'); no StandardScaler, kept as 0/1 indicators",
        ...
        "balancing_strategy": "class_weight tuning, no resampling in this script",
        "selection": {
...
def main():
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            ...
            f"Make sure the CSV file is in the same folder as this script, "
            f"or run the script with --csv-path path/file.csv"
        )

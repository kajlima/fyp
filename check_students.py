#!/usr/bin/env python3
"""
Build an Excel workbook of Zenodo FULL-feature students who were
  1) successfully attacked  (targeted FGSM / PGD hid a real dropout), and
  2) successfully recovered  (the adversarially-trained model catches them again).

This is a rewritten, corrected version. Everything is synchronized with the
ACTUAL final pipeline:
  ...
  MLP
    ...
    - defence  : PGD adversarial training, train/eval epsilon read from file (1.0)

Why the rewrite was needed (bugs in the old version):
  * dataset path was "zenodoo.csv" (typo) instead of "zenodo.csv"
  * feature lists contained resource_events_sem_a/b, which do NOT exist in the data
  * epsilons were hardcoded to values that were never run
    (MLP train 0.2 / eval 0.05 / 0.5, RF attack 1.0 instead of 1.5)
  * it rebuilt the test set with StratifiedGroupKFold(folds 1,2) grouped by
    "student_id_hash", but the real pipeline uses a plain stratified 70/20/10
    train_test_split with no grouping -> the reconstructed test rows did not
    line up with the prediction files, so every row-to-feature join was wrong.

Correct approach used here:
  The prediction CSVs already carry a "student_id" column, so every file is
  joined on student_id. No split reconstruction is needed, and clean raw
  features are pulled straight from zenodo.csv by student_id.

Definitions:
  * successfully attacked = actual dropout AND clean prediction dropout AND
    adversarial prediction non-dropout  (the file's dropout_hidden_tp_to_fn flag).
  * successfully recovered = a successfully-attacked student whose DEFENDED model
    still predicts dropout under attack.
"""
...
# Real FULL feature set (34 columns, no resource_events). Shown as clean context.
FULL_FEATURES = [
...
# Real attackable feature set (21 columns). Only these move under attack.
ATTACKABLE_FEATURES = [
    "credits_enrolled_semester_a", "credits_enrolled_semester_b",
    "total_credits_enrolled_academic_year",
    "completion_rate_one_year_before", "completion_rate_two_years_before",
...
def attach_adv_diffs(table: pd.DataFrame, attack_pred_path: Optional[Path],
    ...
    the attack prediction file. So we tag the raw rows with the prediction file's
    student_id (positional), then join to the table by student_id and compute
    adv - clean diffs on the attackable features.
    """

#!/usr/bin/env python3
"""
Cleaning & preprocessing pipeline for UPV Dropout datasets (2018, 2021, 2022).

Input files are processed independently. The script DOES NOT merge years.
It produces one student-level cleaned CSV per input year plus a QA report.

Main rules implemented:
- Drop row-level credit inconsistencies before aggregation.
- Drop students with studies_start_year > academic_year.
- Compute degree_size before dropping leakage columns.
- Drop student-course rows whose LMS/lms activity values are all blank/NaN.
  A numeric 0 counts as a real value, so rows with 0 are NOT removed.
- Drop rows where nota_asig_hash / course_grade is blank before aggregation.
- Recompute n_courses and courses_mean AFTER all row/course filters.
- Convert monthly LMS/Wi-Fi columns into semester buckets:
  sem_a = Sep-Oct-Nov-Dec-Jan, sem_b = Feb-Mar-Apr-May-Jun, Jul-Aug dropped.
- IMPORTANT FIX FOR 2018 STRUCTURAL BLANK LMS/Wi-Fi BLOCKS:
  keep the semester activity columns in the output, but leave their values blank/NaN;
  do NOT impute 0 into those blank cells, and leave the related missing-flag columns blank/NaN too.
- Final student-level activity safety filters:
  * Drop rows if lms_events_sem_a > 4000 OR lms_events_sem_b > 4000.
  * Then apply the existing general high-activity rule: any selected LMS/lms feature > 5000.
  * Hard validation raises an error if lms_events_sem_a or lms_events_sem_b > 4000 still remains.
- Do not output frac_courses_with_lms or frac_courses_without_grade.
- Translate target labels: abandono_hash A -> dropout, B -> non-dropout.

Note on train-only preprocessing:
Median imputation, p99 outlier capping (winsorization), and StandardScaler are
all done in TRAIN only. This script intentionally does not fit any data-learned
transform on the full yearly outputs; they live in fit_transform_train_only.
Only fixed domain rules are applied during cleaning, since a constant threshold
does not leak.
"""
...
def resolve_input_file(year: int) -> Path:
    """Find the dataset file for a year, including names like dataset_2018_hash(7).csv."""
    candidates = [
        Path(f"dataset_{year}_hash.csv"),
        Path(f"dataset_{year}_hash(7).csv"),
    ]
...
def validate_feature_lists(final: pd.DataFrame, year: int, report: Dict[str, object]) -> List[str]:
    """Make sure every produced column is either an excluded non-feature or assigned to FULL_FEATURES."""
    warnings: List[str] = []
...
def drop_rows_without_any_lms_lms_activity(
    ...
    """
    Drop student-course rows whose LMS/lms monthly activity values are all blank/NaN.
    A numeric 0 is treated as a real value.
    """
...
def block_is_structurally_blank(df: pd.DataFrame, cols: List[str]) -> bool:
    """
    True when a feature block is completely blank for every row and every
    source month in the current yearly dataframe.
    """
    if not cols:
        return True
...
def build_semester_activity_features(
    ...
    """
    Create semester activity features and related missing flags.
    """
...
def clean_one_year(path: Path, year: int, output_dir: Path) -> Dict[str, object]:
    ...
    # -----------------------------------------------------------------------
    # These filters are intentionally placed after all student-level aggregation,
    # column renaming, column ordering, and integer conversion.
    # -----------------------------------------------------------------------

    # Specific lms-events rule:
    # Drop students when EITHER semester-level lms event total is > 4000:
    #   lms_events_sem_a > 4000 OR lms_events_sem_b > 4000
    ...
    # Existing general high-activity rule:
    # Drop students when ANY selected LMS/lms activity feature is > 5000.
    ...
    # Hard validation:
    # After final filters, no lms_events_sem_a / lms_events_sem_b value may remain > 4000.
...
def fit_transform_train_only(train_df: pd.DataFrame, test_df: pd.DataFrame):
    """
    Fit median imputation + p99 outlier caps + StandardScaler on TRAIN ONLY,
    then apply them to TEST.
